...
void uiSpriteRender::draw(){
	...
	for(vector<uiSprite*>::iterator itSprites = sprites.begin();itSprites != sprites.end();++itSprites){
		draw(**itSprites);
		(*itSprites)->setDirty();
		for(vector<uiSprite*>::const_iterator it = (*itSprites)->getChildren().begin() ;it != (*itSprites)->getChildren().end();++it){
			draw(**it);
		}
	}
...
//			//worldMatrix.setOrtho( 0, 1920, getWindowHeight(), 0, -100, 100 );
